...
use statemint_runtime::VerifierId;
...
pub fn statemint_session_keys(keys: AuraId,verifier: VerifierId) -> statemint_runtime::opaque::SessionKeys {
	statemint_runtime::opaque::SessionKeys { aura: keys, verifier }
}
			...
			statemint_genesis(
				...
				vec![
					(
						...
						get_from_seed::<VerifierId>("Alice"),
					)
				],
				vec![
					get_account_id_from_seed::<sr25519::Public>("Alice"),
...
fn statemint_genesis(
	invulnerables: Vec<(AccountId, AuraId, VerifierId)>,
	endowed_accounts: Vec<AccountId>,
	id: ParaId,
	...
	statemint_runtime::GenesisConfig {
		...
		session: statemint_runtime::SessionConfig {
			keys: invulnerables.iter().cloned().map(|x| (
				x.0.clone(), // account id
				x.0.clone(), // validator id
				statemint_session_keys(x.1.clone(),x.2.clone()), // session keys
			)).collect()
		},
		...
		aura_ext: Default::default(),
		parachain_system: Default::default(),
		validator_set: statemint_runtime::ValidatorSetConfig {
			validators: invulnerables.iter().map(|x| x.0.clone()).collect::<Vec<_>>(),
		},
	}
}
